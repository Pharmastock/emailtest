...
// SMTPS Server Configuration (Port 465)
const smtpPort = process.env.SMTP_PORT || 465;
const smtpServer = new SMTPServer({
  secure: true, // SMTPS requires a secure connection
  authOptional: true,
  ...
  async onData(stream, session, callback) {
    ...
    stream.on('end', async () => {
      try {
        ...
        if (!user) {
          console.error(`User not found for recipient email: ${recipientEmail}`);
...
smtpServer.listen(smtpPort, (err) => {
  if (err) {
    ...
    console.log(`SMTP server running securely on port ${smtpPort}`);
  }
});
